...
import { GetArticlesResponse } from 'services/Api'
...
const Home = () => {
  ...
  const fetchArticles = () => {
    return API.articles.articlesList({ Page: 1, Size: 20 }).then((res) => {
      console.log(res)
      setData(res)
    })
  }
  ...
  return (
    <div>
      {data?.map((article) => (
        ...
        >
          <p className={'text-4 font-bold lg:text-[24px] lg:font-semibold'}>
            {article?.title}
          </p>
